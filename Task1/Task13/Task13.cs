...
namespace Task13
{
    internal class Task13
    {
        ...
        public static void Main(string[] args)
        {
            ...
            SwapDiagonals(ref numbers);

            Console.WriteLine($"Array with {n} rows and {m} columns, where diagonals on smallest square swapped:");
            WriteArray(numbers);
        }

        private static void SwapDiagonals(ref int[][] numbers)
        {
            var n = numbers.Length > numbers[0].Length ? numbers[0].Length : numbers.Length;

            for (var i = 0; i < n; i++)
                (numbers[i][i], numbers[i][n - 1 - i]) = (numbers[i][n - 1 - i], numbers[i][i]);
        }
